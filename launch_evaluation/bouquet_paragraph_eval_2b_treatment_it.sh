#!/bin/bash
#SBATCH --job-name=bouquet_paragraph_eval_2b
#SBATCH --error=slurm_logs_xixian/bouquet_paragraph_eval_2b_%j.err
#SBATCH --output=slurm_logs_xixian/bouquet_paragraph_eval_2b_%j.out
#SBATCH -q acc_debug
#SBATCH -c 20
...
#!/bin/bash
#model='/gpfs/projects/bsc88/mt_translation/instructed_models/salamandraTA_7b_extended_mixture_test/checkpoint-1568'
model='/gpfs/projects/bsc88/mt_translation/instructed_models/salamandraTA_2b_v3_mixture1_treatment/'
#model='/gpfs/projects/bsc88/mt_translation/instructed_models/salamandraTA_7b_extended_mixture1_v3/checkpoint-11934'

model_name_dir='salamandraTA_2b_v3_mixture1_treatment'
prompt_style='salamandraTA7B_instruct'
batch_size=2
num_fewshot=0
verbosity='INFO'
...
for pair in "${language_pairs[@]}"; do
    ...
    output_dir="/gpfs/projects/bsc88/mt_translation/mt-evaluation-v3/results/${model_name_dir}/results_${src_abrv}_${tgt_abrv}_bouquet_paragraph.json"

    bash scripts/run_mt_v3.sh "$output_dir" --model hf \
            --model_args "pretrained=${model},trust_remote_code=True,dtype=bfloat16" \
            --tasks "${src_abrv}_${tgt_abrv}_bouquet_paragraph" \
            --num_fewshot $num_fewshot \
            --batch_size $batch_size \
            --verbosity $verbosity \
            --translation_kwargs "src_language=${src_language},tgt_language=${tgt_language},prompt_style=${prompt_style}" \
            --gen_kwargs "max_gen_toks=2000,num_beams=5,early_stopping=True"

    output_dir="/gpfs/projects/bsc88/mt_translation/mt-evaluation-v3/results/${model_name_dir}/results_${tgt_abrv}_${src_abrv}_bouquet_paragraph.json"

    bash scripts/run_mt_v3.sh "$output_dir" --model hf \
            --model_args "pretrained=${model},trust_remote_code=True,dtype=bfloat16" \
            --tasks "${tgt_abrv}_${src_abrv}_bouquet_paragraph" \
            --num_fewshot $num_fewshot \
            --batch_size $batch_size \
            --verbosity $verbosity \
            --translation_kwargs "src_language=${tgt_language},tgt_language=${src_language},prompt_style=${prompt_style}" \
            --gen_kwargs "max_gen_toks=2000,num_beams=5,early_stopping=True"
