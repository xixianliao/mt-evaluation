#!/bin/bash
#SBATCH --job-name=flores_eval_v3_bleurt
#SBATCH --error=slurm_logs_xixian/eval_v3_bleurt_flores_%j.err
#SBATCH --output=slurm_logs_xixian/eval_v3_bleurt_flores_%j.out
#SBATCH -q acc_debug
#SBATCH -c 20
...
for pair in "${language_pairs[@]}"; do
    ...
    output_dir="/gpfs/projects/bsc88/mt_translation/mt-evaluation-v3/results/${model_name_dir}/results_${src_abrv}_${tgt_abrv}_flores+_devtest.json"
    ...
    output_dir="/gpfs/projects/bsc88/mt_translation/mt-evaluation-v3/results/${model_name_dir}/results_${tgt_abrv}_${src_abrv}_flores+_devtest.json"

    bash scripts/run_mt_v3.sh "$output_dir" --model hf \
            --model_args "pretrained=${model},trust_remote_code=True,dtype=bfloat16" \
            --tasks "${tgt_abrv}_${src_abrv}_flores+_devtest" \
            --num_fewshot $num_fewshot \
            --batch_size $batch_size \
            --verbosity $verbosity \
